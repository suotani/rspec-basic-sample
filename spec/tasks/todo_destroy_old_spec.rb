...
RSpec.describe Todo do
  ...
  describe 'todo:destroy_old' do
    ...
    context "target new and lower priority" do
      let!(:todo1){create(:todo, priority: 1,created_at: Time.zone.today + 1.day)}
      it "not destroy" do
    ...
    context "target old and lower priority" do
      let!(:todo1){create(:todo, priority: 1,created_at: Time.zone.today - 6.day)}
      it "destroy" do
    ...
    context "target old and high priority" do
      let!(:todo1){create(:todo, priority: 2,created_at: Time.zone.today - 6.day)}
      it "not destroy" do
        expect{ task.execute() }.to change(Todo, :count).by(0)
      end
    end
